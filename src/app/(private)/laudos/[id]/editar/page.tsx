...
import { getLaudoById } from "@/features/laudos/services/laudo.services";
...
export default function EditarLaudoPage() {
  ...
  const { data: laudo, isLoading } = useQuery({
    queryKey: queryKeys.laudos.byId(id),
    queryFn: () => getLaudoById(id),
  });
  ...
  return (
    <div className="w-full space-y-6">
      <div className="space-y-4">
        <button
          onClick={() =>
            router.push(pacienteId ? `/pacientes/${pacienteId}` : "/laudos")
          }
      ...
      {isLoading || isLoadingModules ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            A edição de laudos só fica disponível para organizações com a
        ...
        <div className="w-full bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          <LaudoForm pacienteId={pacienteId} laudo={laudo} />
        </div>
      ) : (
