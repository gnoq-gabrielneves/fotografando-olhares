...
export function ResetForm() {
  ...
  if (submitted) {
    return (
      <div className="text-center space-y-4">
        <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-cyan-950 border border-cyan-800/50">
          <CheckCircle className="w-6 h-6 text-cyan-400" />
        </div>
        <div>
          <h2 className="text-lg font-medium text-white">E-mail enviado!</h2>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <a
        href="/login"
        className="flex items-center justify-center gap-1 text-xs text-slate-500 hover:text-slate-300 transition-colors"
      >
        <ArrowLeft className="w-3 h-3" />
        Voltar para o login
      </a>
